...
def main(input_data: Input) -> Output:
    ...
    try:
        # For agents 2, 3, 4, 7
        res = PatternDetectionAgent.run(input_data.case_data)
    except TypeError:
        try:
            # Agent 5 and 8 might have different signatures
            # We'll just pass risk_score=85 as default if needed
            res = PatternDetectionAgent.run(input_data.case_data, 85)
        except Exception:
            try:
                # Agent 8 needs form_data, narrative, case_id
                res = PatternDetectionAgent.run(input_data.case_data, "Auto narrative", "CAS-123")
